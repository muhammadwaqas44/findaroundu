<script>
    function getReviews(id) {
        ...
        baseUrl = "{{url('user/business/')}}" + "/" + id + "/reviews";
        ...
        $.get(baseUrl, function (data, status) {
         ...
         //   console.log(data);\
            if(liHtml == "")
            {
                document.getElementById('posts_url_review').innerHTML = `<div class="col-lg-12">No Review posted</div>`;
            }else{
    ...
    @if(Auth::check())
    function addLike(reviewId = null, commentId = null, addId = null) {
        userId = {{Auth::user()->id}};
        if (reviewId != null) {
        ...
        if (addId != null) {
            baseUrl = "{{url('user/review-comment')}}" + "/" + addId + "/like";
        }
