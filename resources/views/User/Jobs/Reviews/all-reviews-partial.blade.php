...
<div class="nxt-listing" id="write-review-section">
    ...
    <div class="left-main-div">
        ...
        {{\App\Helpers\MapHelper::mapHelper($data['reviews'])}}
        <small>({{$data['reviews']->count()}})</small>
        <a class="leave-review1" @if(!Auth::check()) href="{{route('login')}}" @endif>Leave Review <i class="fas fa-level-down-alt"></i></a>
    </div>
...
<div class="nxt-listing" id="user-review-section">
    <h3 class="watch-vedio"><a @if(!Auth::check()) href="{{route('login')}}" @endif>Leave Review </a></h3>
    <div class="row">
        ...
        <div class="col-md-7">
            @if(Auth::check())
                <form method="post" action="{{route('user.give-review')}}"> @csrf
                    ...
                    <input name="add_id" value="{{$data['job_detail']->id}}"  style="display:none;">
                    <ul class="last-ul">
                        <li>
