<form method="post" action="{{route('admin.edit-customer',[$data['user_detail']->id])}}">
    @csrf
                        ...
                        <span class="search-drop-down5">
                            <select class="form-control enter-text-field" name="main_country_id">
                                @foreach($data['countries'] as $country)
                                    @if(!empty($data['user_detail']->address))
                                        <option  @if($data['user_detail']->address->main_country_id == $country['id'] ) selected @endif value="{{$country['id']}}">{{$country['name']}}</option>
                               @endif
                                @endforeach
                            </select>
                        </span>
                        ...
                        <span class="search-drop-down5">
                            <select class="form-control" name="main_city_id">
                                @foreach($data['cities'] as $city)
                                    @if(!empty($data['user_detail']->address))
                                    <option @if($data['user_detail']->address->main_city_id == $city['id'] ) selected @endif value="{{$city['id']}}">{{$city['name']}}</option>
                                   @endif
                                @endforeach
                            </select>
                        </span>
                ...
                <div class="row">
                    ...
                    <div class="col-7">
                        <input type="text" id="address"
                               value="
                               @if(!empty($data['user_detail']->address))
                            {{$data['user_detail']->address->address}}
                                       @endif"
                               name="address" placeholder="Enter Address"
                                    class="enter-text-field"  autocomplete="off" required>
                    </div>
                </div>
