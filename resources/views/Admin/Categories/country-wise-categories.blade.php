                                ...
                                <div class="tab-pane active">
                                    <div class="first-tab-form-main">
                                        <div class="border-box">
                                            <span class="holiday-text">Country Wise Categories</span>
                                            <div class="row">
                    ...
                    <div class="row" style="margin-top: 30px;">
                        <div class="col-lg-6">
                            <div class="details-tabs-main">
                                <div class="first-tab-form-main">
                                    <div class="border-box">
                                        <span class="holiday-text">All {{$type}} Categories</span>
                                        <ul class="cat_scroll_area main_cat_list">
                                            @foreach($data["all_categories"] as $category)
                                                @php
                                                    $catObj = $category["category_object"];
                                                    $catChilds = $category["category_childs"];
                                                @endphp
                                                <li>
                                                    <div class="cat_area border-box" id="drag-{{$catObj->id}}" draggable="true" ondragstart="drag(event)" style="{{(in_array($catObj->id, $data["count_cat_ids"]))? "display:none": ""}}">
                                                        <form class="updateCatForm">
                                                            @csrf
                                                            <input type="hidden" name="id" value="{{$catObj->id}}">
                                                            <div class="car_img_area">
                                                                <img draggable="false" class="img-thumbnail" src="{{url($catObj->profile_image)}}" alt="{{$catObj->name}}">
                                                            </div>
                                                            <input type="file" name="profile_image" class="catImg">
                                                            <div class="form-group">
                                                                <input type="text" name="name" class="" placeholder="Category Name" value="{{$catObj->name}}" required>
                                                    ...
                                                    <ul class="cat_child_list">
                                                        @foreach($catChilds as $catChild)
                                                            <li>
                                                                <div class="cat_area border-box" id="drag-{{$catChild->id}}" draggable="true" ondragstart="drag(event)" style="{{(in_array($catChild->id, $data["count_cat_ids"]))? "display:none": ""}}">
                                                                    <form class="updateCatForm">
                                                                        @csrf
                                                                        <input type="hidden" name="id" value="{{$catChild->id}}">
                                                                        <div class="car_img_area">
                                                                            <img draggable="false" class="img-thumbnail" src="{{url($catChild->profile_image)}}" alt="{{$catChild->name}}">
                                                                        </div>
                                                                        <input type="file" name="profile_image" class="catImg">
                                                                        <div class="form-group">
                                                                            <input type="text" name="name" class="" placeholder="Category Name" value="{{$catChild->name}}" required>
                                                                            <button type="submit" class="btn-sm btn btn-primary catUpdateBtn">Update</button>
                                                                        </div>
                                                                    </form>
                                                                </div>
                        ...
                        <div class="col-lg-6">
                            <div class="details-tabs-main">
                                <div class="first-tab-form-main">
                                    <div class="border-box">
                                        <span class="holiday-text">Selected {{$type}} Categories (<span id="liCounter">{{sizeof($data["country_categories"])}}</span>)</span>
                                        <form id="selectedCatForm">
                                            ...
                                            <ol class="cat_scroll_area main_cat_list" id="dropEvent" ondrop="drop(event)" ondragover="allowDrop(event)">
                                                @foreach($data["country_categories"] as $category)
                                                    <li>
                                                        <div class="border-box">
                                                            <input type="hidden" name="id[]" value="{{$category->category->id}}">
                                                            <button type="button" class="btn-sm btn btn-danger removeElement" data-id="drag-{{$category->category->id}}">Remove</button>
                                                            <div class="car_img_area">
                                                                <img draggable="false" src="{{url($category->category->profile_image)}}" alt="{{$category->category->name}}" class="img-thumbnail">
                                                            </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $("#selectBtn").click(function (e) {
                ...
                window.location.href = "/admin/country-wise-categories/"+countryId;
                showLoader();
            });
            ...
            $(".updateCatForm").submit(function (e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('admin.category-data-update')}}",
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
            ...
            $("#selectedCatForm").submit(function (e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('admin.country-selected-category-update')}}",
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
        ...
        function drop(ev) {
            ...
            if(counter < 18) {
                ...
                errorMsg("Please remove already added category for add more.");
            }
        }
